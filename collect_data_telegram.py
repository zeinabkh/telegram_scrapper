...
async def main():
    ...
    #                 'date': datetime.datetime.now()},
                   {'name': "bours_rezaeii",
                    'date': datetime.datetime.now()},
                   {'name': "گروه فعالان بازار بورس تهران",
                    'date': datetime.datetime.now()}]
    for group in group_list:
        ...
        users_id = []
        print(group['name'])
        # excel_data_file = xlwr.Workbook(group['name'] + ".xlsx")
        ...
        date_sheet = str(group['date'].year)+"-" + str(group['date'].month)+"-" +str(group['date'].day)
                ...
                if message_date == previous_date:
                    if message.media is None and message.from_id is not None:
                        ...
                        try:
                            ...
                            user = await client.get_entity(user_id)
                        # date_sheet.write(i, 0, message.from_id.user_id)
                    ...
                    if True:
                        try:
                            ...
                            user = await client.get_entity(message.from_id.channel_id)
                            name = group['name']
                        ...
                        message_users.append(user.username)
                        users_names.append(name)
                        users_id.append(message.from_id.user_id)
                        i += 1
                    print(day)
                    if day > 80:
                        break
